import {  mainPool,analysisPool, ORDERS_TABLE } from "../config/db";
    ...
    for (const row of result.rows) {
      await analysisPool.query(
        ...
        [
          ...
          row.destination_redeem_tx_hash ? row.destination_redeem_block_number : null, // Swapped to user
          row.source_refund_tx_hash ? row.source_refund_block_number : null,
          row.destination_init_block_number,
          row.source_redeem_tx_hash ? row.source_redeem_block_number : null, // Swapped to cobi
          ...
          row.destination_redeem_tx_hash ? row.destination_redeem_tx_hash : null, // Swapped to user
          row.source_refund_tx_hash ? row.source_refund_tx_hash : null,
          row.destination_init_tx_hash ? row.destination_init_tx_hash : null,
          row.source_redeem_tx_hash ? row.source_redeem_tx_hash : null, // Swapped to cobi
